...
namespace Compress.Core
{
    public class BitWriter : IBitWriter
    {
        ...
        {
            var arrBytes = new byte[this.bytes.Count];
            Array.Copy(this.bytes.ToArray(), arrBytes, this.bytes.Count);
            this.bytes.Clear();
            this.offset = 0;
            return arrBytes;
        }
        ...
        public byte[] GetRoundBytes()
        {
            ...
            {
                var arrBytes = new byte[this.bytes.Count-1];
                Array.Copy(this.bytes.ToArray(), arrBytes, this.bytes.Count-1);
                this.bytes.RemoveRange(0, this.bytes.Count - 1);
                return arrBytes;
            }                
        }
